...
const Room = (props) => {
  ...
  const currentUser = sessionStorage.getItem('user') || `User ${Math.floor(Math.random() * 1000)}`
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then((stream) => {
      ...
      socket.emit('BE-join-room', { roomId, userName: currentUser, fullName: name })
      socket.on('FE-user-join', (users) => {
        ...
        users.forEach(({ userId, info }) => {
          ...
          let { userName, video, audio, fullName } = info
      ...
      <VideoAndBarContainer>
        <VideoContainer>
          ...
          <VideoBox className={`width-peer${peers.length > 8 ? '' : peers.length}`}>
            {userVideoAudio['localUser'].video ? null : <UserName>{name}</UserName>}
            <FaIcon className="fas fa-expand" />
